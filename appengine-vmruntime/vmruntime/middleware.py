...
RESERVED_ENV_KEYS = {
    ...
    'HTTPS': '',
    # REMOTE_ADDR is reserved but has special case handling.
    #   'REMOTE_ADDR': '',
    'REQUEST_ID_HASH': '',
    'REQUEST_LOG_ID': '',
    'USER_EMAIL': '',
    ...
    'USER_NICKNAME': '',
    'USER_ORGANIZATION': '',
    'SERVER_SOFTWARE': 'Google App Engine/0.70',
}
...
def reset_environment_middleware(app, frozen_environment, frozen_user_env,
    ...
    This requires a single-threaded webserver, or for os.environ to be patched
    to be thread-local.

    Args:
        app: The WSGI app to wrap.
        frozen_environment: An iterable of (key, value) tuples that can be used
            to populate os.environ with the initial state of the environment.
        ...
        frozen_user_env: An iterable of (key, value) tuples that can be used to
            populate os.environ with environment variables specified in
            app.yaml.
        frozen_env_config_env: An iterable of (key, value) tuples that can be
            used to populate os.environ with configuration-dependent env
            variables.
    ...
    def reset_environment_wrapper(request):
        ...
        # Populate os.environ in order, so that later steps overwrite
        # conflicting keys in previous steps.

        # Add in user env variables specified in app.yaml. These are added
        # first because they should not take precedence over any conflicting
        # key.
        ...
        # Patch the wsgi environment to set REMOTE_ADDR to equal the origin IP
        # and not the IP of the proxy server. This mimics existing GAE
        # behavior.
        ...
        # Add reserved keys, which draw from wsgi_env as well. These have a
        # very high priority and so are added nearly last.
...
def request_environment_for_wsgi_env(wsgi_env):
  ...
  """
    ...
    return {key: value
            for key, value in wsgi_env.iteritems()
...
def get_env_to_hide_service_bridge(wsgi_env):
    """Generate a dictionary of environment variables to hide the service
    bridge.
    ...
    # SERVER_PORT, respectively.  However, this is an implementation detail
    # that should not be shown to user code. Instead, we'll rely on the
    # HTTP Host header to retrieve the hostname used in the original request.
    # This mimics the behavior of non-VM App Engine.
    ...
    if https == 'off':
        ...
        logging.warning(
            'Unrecognized value for HTTPS (%s), won\'t modify SERVER_PORT',
            https)
