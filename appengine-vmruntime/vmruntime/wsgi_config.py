...
def app_for_script(script):
    ...
    try:
        ...
        # wsgi.LoadObject guarantees this is either None or ImportError.
        if err:
            raise err
    except ImportError:
        # Despite returning an error object sometimes, LoadObject will other
        # times just raise an exception. By raising the err object if it
        # exists, we catch either.
...
def static_app_for_handler(handler, expiration):
    ...
    if not files:
        if handler.static_dir:
            # If static_files is not set, convert static_dir to static_files
            # and also modify the url regex accordingly. See the appinfo.URLMap
            ...
            # Neither static_files nor static_dir is set; log an error and
            # return.
...
def load_user_scripts_into_handlers(app_info):
    ...
    for handler in app_info.handlers:
        ...
        # A static files directive, either with static_files or static_dir.
        else:
            if handler.static_files:
                url_re = handler.url
            else:  # This is a "static_dir" directive.
                url_re = static_dir_url_re(handler)
            expiration = (handler.expiration or app_info.default_expiration or
    ...
    logging.info('Parsed handlers: %r', [
        loaded_url_re for (loaded_url_re, _) in loaded_handlers])
...
def user_env_vars_from_appinfo(appinfo_ext):
    ...
    Args:
        appinfo_ext: The configuration info (a parsed yaml object) as generated
            by get_module_config()
