...
class MockedWsgiHandler(object):
    ...
    # For grabbing the user's python25 app.
...
def load_legacy_scripts_into_handlers(handlers):
    ...
    """
    loaded_handlers = [
        (x.url, legacy_app_for_script(x.script.replace('$PYTHON_LIB/', '')))
        for x in handlers
    ]
...
def legacy_app_for_script(script):
    ...
    Args:
        script: A script specification from a python27 app.yaml. e.g.
        my.module.app
...
def stub_wsgi_utils(output_app_holder):
    ...
    # versions, IISCGIHandler is added. But we can't really stub that here
    # since we're running with python2.7.
